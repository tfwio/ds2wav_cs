...
namespace on.drumsynth2
{
  ...
  /// </summary>
  public partial class DrumSynthFloat
  {
    ...
    const float Pi      = (float)Math.PI; //6.285714f; //Math.PI;
    const float TwoPi   = Pi * 2;
    const float TwoPiDenom = 1 / Pi * 2;
    //const float TwoPi = 6.285714f;
    ...
    float LoudestEnv()
    {
      ...
      while (i<5) //2
      {
        if(chkOn[i]==1 && sliLev[i]>loudest) loudest=(float)sliLev[i];
        i++;
      }
    ...
    void getParseEnv(int env, string keyValue)
    {
      ...
      for (i=0; i < keyValues.Length; i++)
      {
        ...
        envpts[env,0,i] = float.Parse(vn[0]);
        if (!Fs.Equals(44100)) envpts[env,0,i] = envpts[env,0,i] / 44100 * Fs;
        envpts[env,1,i] = float.Parse(vn[1]);
      }
    ...
    float waveform(float ph, long form)
    {
    ...
    float waveform(float ph, int form)
    {
      ...
      {
        case 0: //sine
          w = FloatMath.sin(ph % TwoPi);
          break;
        case 1: //sine^2
          w = FloatMath.fabs(2.0f * FloatMath.sin((0.5f * ph) % TwoPi)) - 1.0f; break;
        case 2: //tri
        ...
        case 3: //saw
          ...
          break;
        default: //square
          w = (FloatMath.sin(ph % TwoPi) > 0.0) ? 1.0f : -1.0f;
          break;
      }
      return w;
    }
    ...
    public int do_ds2wav(string dsfile, string wavfile, int sampleRate=44100)
    {
      ...
      phi = new float[BUFFER_SIZE];
      Fs=sampleRate;
      envpts = new float[8,3,EnvelopeCountMax];
      ...
      float a, b=0.0f, c=0.0f, d=0.0f, g, TT=0.0f, TTT, TL, NL;
      // Tone Freq 1 and 2
      float F1, F2, Fsync;
      float TphiStart=0.0f, Tphi, TDroopRate, ddF, DAtten, DGain;
      
      long  BON, BON2, BFStep, BFStep2, botmp;
      float BdF=0.0f, BdF2=0.0f, BPhi, BPhi2, BF, BF2, BQ, BQ2, BL, BL2;

      long  OON, OF1Sync=0, OF2Sync=0, OMode, OW1, OW2;
      float Ophi1, Ophi2, OF1, OF2, OL, Ot=0f, OBal1, OBal2, ODrive;
      float Ocf1, Ocf2, OcF, OcQ, OcA;
      float[,] Oc = new float[6,2];  //overtone cymbal mode
      float Oc0=0.0f, Oc1=0.0f, Oc2=0.0f;

      float MFfb, MFtmp, MFres, MFin=0.0f, MFout=0.0f;
      float DownAve;
      ...
      DGain = FloatMath.pow(10.0f, 0.05f * DsPgm.General.Level);
      MasterTune = DsPgm.General.Tuning;
      MasterTune = FloatMath.pow(1.0594631f, MasterTune + mem_tune);
      
      MainFilter = 2 * DsPgm.General.Filter;
      MFres = 0.0101f * DsPgm.General.Resonance;
      MFres = FloatMath.pow(MFres, 0.5f);
      ...
      else
      {
        a = 1.0f;
        b = -NT / 50.0f;
        c = FloatMath.fabs((float)NT) / 100.0f;
        g = NL;
      }
      ...
      // FMathHelper.srand(1); //fixed random sequence isn't enabled
      // ---------------------------------------------------
      // Tone
      // ---------------------------------------------------
      ...
      if(FloatMath.fabs(F1)<0.001f) F1=0.001f; //to prevent overtone ratio div0
      F2 = MasterTune * TwoPi * DsPgm.Tone.F2 / Fs;
      // Fsync = F2; // not referenced
      TDroopRate = (float)DsPgm.Tone.Droop;
      if(TDroopRate>0.0f)
      {
        TDroopRate = FloatMath.pow(10.0f, (TDroopRate - 20.0f) / 30.0f);
        TDroopRate = TDroopRate * -4.0f / envData[1,MAX];
        TDroop = 1;
        F2 = F1+((F2-F1)/(1.0f-FloatMath.exp(TDroopRate * envData[1,MAX])));
        ddF = F1 - F2;
      }
      else ddF = F2-F1;
      
      Tphi = (float)DsPgm.Tone.Phase / 57.29578f; //degrees>radians
      ...
      ODrive = FloatMath.pow(OBal2, 3.0f) / FloatMath.pow(50.0f, 3.0f);
      OBal2 *= 0.01f;
      OBal1 = 1.0f - OBal2;
      Ophi1 = Tphi;
      Ophi2 = Tphi;
      ...
      BQ = (float)BFStep;
      BQ = BQ * BQ / (10000.0f-6600.0f*(FloatMath.sqrt(BF)-0.19f));
      ...
      BQ2 = (float)BFStep2;
      BQ2 = BQ2 * BQ2 / (10000.0f - 6600.0f * (FloatMath.sqrt(BF2) - 0.19f));
      ...
      if(DiON==1)
      {
        ...
        DAtten = FloatMath.pow(2.0f, 2.0f * DsPgm.Distortion.Bits);
        DGain = DAtten * DGain * FloatMath.pow(10.0f, 0.05f * DsPgm.Distortion.Clipping);
      }
      // ---------------------------------------------------
      // Envelopes
      // ---------------------------------------------------
      randmax = 1.0f / FloatMath.RAND_MAX; randmax2 = 2.0f * randmax;
      ...
      using (var writer = new BinaryWriter(mrStream))
      {
        ...
        tpos = 0;
        //byte[] wave = new byte[BUFFER_SIZE];
        while(tpos < Length)
        {
          ...
          if(NON==1) //noise
          {
            for(t=tpos; t<=tplus; t++)
            {
              ...
              x[2] = x[1];
              x[1] = x[0];
              x[0] = (randmax2 * FloatMath.rand()) - 1.0f;
              TT = a * x[0] + b * x[1] + c * x[2] + d * TT;
              
              DF[t - tpos] = FloatMath.lim(TT * g * envData[2,ENV]);
            }
          ...
          if(TON==1) //tone
          {
            ...
            if(TDroop==1) { for(t=tpos; t<=tplus; t++) phi[t - tpos] = F2 + (ddF * FloatMath.exp(t * TDroopRate)); }
            ...
            for(t=tpos; t<=tplus; t++)
            {
              ...
              Tphi = Tphi + phi[totmp];
              DF[totmp] = FloatMath.lim(
                DF[totmp] + (
                  TL * envData[1,ENV] * FloatMath.sin(
                    FloatMath.fmod(Tphi,TwoPi)
                   )
                 )
               );//overflow?
            }
          ...
          if(BON==1) //noise band 1
          {
            ...
            {
              Envy(t,5);
              if((t % BFStep) == 0) BdF = randmax * FloatMath.rand() - 0.5f;
              BPhi = BPhi + BF + BQ * BdF;
              botmp = t - tpos;
              DF[botmp] = FloatMath.lim(DF[botmp] +FloatMath.cos(FloatMath.fmod(BPhi,TwoPi)) * envData[5,ENV] * BL);
            }
          ...
          if(BON2==1) //noise band 2
          {
            ...
            {
              Envy(t,6);
              if((t % BFStep2) == 0) BdF2 = randmax * FloatMath.rand() - 0.5f;
              BPhi2 = BPhi2 + BF2 + BQ2 * BdF2;
              botmp = t - tpos;
              DF[botmp] = FloatMath.lim(DF[botmp] + FloatMath.cos(FloatMath.fmod(BPhi2,TwoPi)) * envData[6,ENV] * BL2);
            }
          ...
          for (t=tpos; t<=tplus; t++)
          {
            ...
            if (MainFilter==1) //filter overtones
            {
              ...
              // can we classify this filter DF
              // We know expect MFtmp to resolve our frequency however
              // it will be converted from a threshold value

              if(MFtmp >0.2f) MFfb = 1.001f - FloatMath.pow(10.0f, MFtmp - 1);
              ...
              DF[t - tpos] = FloatMath.lim(DF[t - tpos] + (MFout - (HighPass * Ot)));
            } //filter overtones
            else if(MainFilter==2) //filter all
            {
              ...
              DF[t - tpos] = FloatMath.lim(MFout - (HighPass * (DF[t - tpos] + Ot)));
              
            } //filter all
            else DF[t - tpos] = FloatMath.lim(DF[t - tpos] + Ot); //no filter
          ...
          if(DiON==1) //bit resolution
          {
            ...
            {
              DF[j] = FloatMath.lim(DGain * (int)(DF[j] / DAtten));
            }
          ...
          else for(j=0; j<BUFFER_SIZE; j++) DF[j] = FloatMath.lim(DF[j] * DGain);
    ...
    internal static bool IsExtension(string inputfile, string extIn = ".ds")
    {
      return Path.GetExtension(inputfile).ToLower() == extIn;
    }
    /// <summary>
    /// Gets the out file name depending on sampling frequency such as 44,100 samples p/sec.
    /// </summary>
    /// <returns>The out file.</returns>
    /// <param name="inputfile">Inputfile.</param>
    /// <param name="sampleRate">Sample rate.</param>
    /// <param name="extIn">Ext in.</param>
    /// <param name="extOut">Ext out.</param>
